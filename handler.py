...
def handler(job):
    ...
    # Streamlined command array matching the absolute latest FaceFusion CLI syntax specifications
    command = [
    ...
    try:
        # Forces execution and safely intercepts underlying system-level framework failures
        ...
        return {
            "success": False, 
            "error": "FaceFusion crashed internally",
            "stdout": e.stdout, 
            "stderr": e.stderr
        }
        ...
        return {
            "success": False,
            "error": f"System execution engine failure: {str(sys_err)}"
        }

    if not os.path.exists(output_path):
        return {
            "success": False, 
            "error": "Output image file was not generated by FaceFusion engine",
    ...
    try:
        ...
        return {"success": False, "error": f"Encoding output matrix failed: {str(e)}"}
